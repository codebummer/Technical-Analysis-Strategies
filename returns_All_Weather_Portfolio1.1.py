...
for yearend in yearly_prices.index:    
    ...
    # off_holdings = off_values/yearly_prices.loc[yearly_prices.index[0]] 
    # # off_holdings = off_holdings.apply(np.floor).astype('int').to_frame(name=yearly_prices.index[0]).T #incorrect
    # off_holdings = off_holdings.round(decimals=1).astype('int').to_frame(name=yearly_prices.index[0]).T
    # holdings = pd.concat([holdings, off_holdings])

    off_values = values.loc[yearend] - weights/100*values.loc[yearend].sum()
    off_ratio = values.loc[yearend] / (weights/100*values.loc[yearend].sum())    
    off_qty = (off_values/yearly_prices.loc[yearend]).round(decimals=1).astype('int')    
    rebalance_ratio = - off_values/off_values.abs().sum() # buy lost assets and sell gained assets. To do it, multiply - to invert the ratio
    off_amounts = off_qty*yearly_prices.loc[yearend]
    corrections = (off_amounts.sum()*rebalance_ratio)/yearly_prices.loc[yearend]
    # off_qty.sum()*rebalance_ratio # the values are different from the above
    corrections = corrections.round(decimals=1).astype('int')
    
    
    (corrections.sort_values()*yearly_prices.loc[yearend]).cumsum()
    holdings.loc[yearend] + corrections
    
    
    
    off_values/off_values.abs().sum()
        
    
    
    sorted = off_qty.sort_values().loc[off_qty.sort_values().abs()>1]
    buy = sorted.loc[sorted<-1].round(decimals=1).astype('int')
    sell = sorted.loc[sorted>1].round(decimals=1).astype('int')
    
    returns = ((sell*yearly_prices.loc[yearend,sell.index]).sum()+(buy*yearly_prices.loc[yearend,buy.index]).sum())
    returns*(1-off_ratio) # 1-off_ratio: ratio to be bought (+) or sold (-)

off_qty

off_values = values.iloc[0,:] - weights/100*values.iloc[0,:].sum()
off_ratio = values.iloc[0,:]/(weights/100*values.iloc[0,:].sum())

off_qty = off_values/yearly_prices.iloc[1,:]
sorted = off_qty.sort_values().loc[off_qty.sort_values().abs()>0.5]
buy = sorted.loc[sorted<-1].round(decimals=1).astype('int')
sell = sorted.loc[sorted>1].round(decimals=1).astype('int')
...
# holdings.loc[yearend,buy.index] - buy # buy has minus values, so - buy not + buy
# holdings.loc[yearend,sell.index] - sell # sell has plus values
...
values.iloc[0,:]/(weights/100*values.iloc[0,:].sum())

off_ratio.loc[off_ratio.abs()>0.5]

weights/100*yearly_prices.iloc[1,:].sum()

weights/100*values.iloc[0,:].sum()
yearly_prices.iloc[1,:]
