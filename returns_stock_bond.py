...
for yearend in yearly_prices.index:    
    ...
    holdings.loc[yearend] = holdings.loc[yearend] + rebalance_qty * rebalance_assets - off_qty.clip(lower=0)

holdings.loc[holdings.index.year==2011] + rebalance_qty.loc[rebalance_assets] - off_qty.clip(lower=0)
rebalance_qty*rebalance_assets
...
risk_free = yearly_ret.sub(yearly_ret['IEF'], axis='index')
...
all_year_class_sharpe = (all_year_returns-all_year_returns['IEF'])/all_year_class_stds
all_year_total_sharpe = (all_year_returns.sum()-all_year_returns['IEF'])/all_year_total_stds
...
gld = math.floor(invested/prices['IEF'][0])

sns.set()
fig, ax = plt.subplots(1)
ax.plot((ratio*prices).sum(axis='columns'))
ax.plot(daily_portfolio_values.sum(axis='columns'))
ax.plot(prices['SPY']*spy)
ax.plot(prices['IEF']*gld)
ax.legend(['Non-Weighted', 'Weighted', 'SPY', 'IEF'])
plt.show()
