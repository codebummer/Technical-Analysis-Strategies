...
cumprod = prices.pct_change().add(1).cumprod()
returns = np.power(cumprod, 1/len(prices)) - 1
total_returns = returns.iloc[-1,:]

stds = prices.pct_change().std()
covs = prices.pct_change().cov()
...
sns.heatmap(corrs, cmap='YlGnBu', annot=True, linewidths=0.1, ax=ax2)
# fig2.tight_layout()
plt.show()
...
for _ in range(200_000):
    ...
    returns = np.dot(weights, total_returns)
    risk = np.sqrt(np.dot(weights.T, np.dot(corrs, weights)))
...
for i, s in enumerate(stocks.keys()):
    portfolio[s] = [weight[i] for weight in port_weights]

df = pd.DataFrame(portfolio)
df = df[['Returns', 'Risk', 'Sharpe'] + [s for s in stocks.keys()]]
